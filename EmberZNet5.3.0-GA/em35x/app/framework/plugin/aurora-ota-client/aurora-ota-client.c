...
#include "app/framework/plugin/aurora-ota-client/aurora-ota-client.h"
#include "app/framework/plugin/aurora-ota-client-policy/aurora-ota-client-policy.h"
#include "aurora-ota-client-signature-verify.h"
#include "aurora-ota-client-page-request.h"
...
#if defined(EMBER_AF_PLUGIN_AURORA_OTA_CLIENT_SET_IMAGE_STAMP) \
    && !defined(EZSP_HOST) \
...
extern boolean isEepromAvailable(void);  
...
// we use this value for the next request.
    //MN Commented out per Gary's instructions to reduce upgrade delay to 1s
//#define CALCULATE_TIME_ERROR_UPGRADE_END_RESPONSE_DELAY_MS (1 * HOURS_IN_MS)
#define CALCULATE_TIME_ERROR_UPGRADE_END_RESPONSE_DELAY_MS (1 * SECONDS_IN_MS)
...
#if defined(EMBER_AF_PLUGIN_AURORA_OTA_CLIENT_USE_PAGE_REQUEST)
...
// bits and limit the number retrieved from halCommonGetRandom()
...
{
    //GB - Initialise the OTA cluster only if the eeprom is available.
    if (!isEepromAvailable()) 
    {
        emberAfDebugPrintln("Not initialising OTA");
        emAfOtaClientStop();  // stop ota tick.
        emberAfEndpointEnableDisable(3, FALSE);  // Disable the OTA ep.        
        return;
    }    
...
#if defined(USE_IMAGE_STAMP)
  {
    ...
    int8u data[IMAGE_STAMP_SIZE];
    MEMCOPY(data, halAppAddressTable.imageStamp, IMAGE_STAMP_SIZE);
    emberAfWriteAttribute(myEndpoint,
                          ZCL_OTA_BOOTLOAD_CLUSTER_ID,
...
void emberAfPluginAuroraOtaClientStackStatusCallback(EmberStatus status)
{
  if (!isEepromAvailable()) 
  {
      return;  //GB ota not active if no external flash.
  }
  
#if defined(EMBER_AF_PLUGIN_AURORA_OTA_CLIENT_AUTO_START)
  int16u randomDelaySeconds;
  if (status != EMBER_NETWORK_UP
      || currentBootloadState != BOOTLOAD_STATE_NONE) {
    return;
  }
  randomDelaySeconds = (halCommonGetRandom()
  ...
  setTimer(randomDelaySeconds << 10);  // multiply by 1024 to get approx ms.
#endif
}
...
static BootloadState determineDownloadFileStatus(void)
{
  ...
  if (status == EMBER_AF_OTA_STORAGE_PARTIAL_FILE_FOUND) {
    ...
    //MN EDITED OUT after Gary's email
/* 
    ...
    */
    ...
    if (currentVersionInfo.firmwareVersion != currentDownloadFile.firmwareVersion) {
      ...
      otaPrintln("File in storage is same as current running version (0x%4X)",
                 currentVersionInfo.firmwareVersion);
      emberAfOtaStorageClearTempDataCallback();  // erase eeprom.
    }
  } else {
                                  ...
                                  EmberAfClusterCommand* message)
{
  ...
  case ZCL_IMAGE_NOTIFY_COMMAND_ID: {
    ...
    if ((currentBootloadState != BOOTLOAD_STATE_QUERY_NEXT_IMAGE) && (currentBootloadState != BOOTLOAD_STATE_NONE)) {
      otaPrintln("Got unexpected %p.  Ignored.", "Image notify");
      emberAfDebugPrint("Image Notify received %x\r\n", currentBootloadState);
  ...
  switch(cmdId) {
    ...
    case ZCL_IMAGE_PAGE_REQUEST_COMMAND_ID: 
      ...
      if (cmdId == ZCL_IMAGE_PAGE_REQUEST_COMMAND_ID) {
        emberAfPutInt16uInResp(EMBER_AF_PLUGIN_AURORA_OTA_CLIENT_PAGE_REQUEST_SIZE);        
                           ...
                           boolean queryErrorOccurred)
{
  ...
  if (queryErrorOccurred) {
    errors++;
    if (errors >= EMBER_AF_PLUGIN_AURORA_OTA_CLIENT_QUERY_ERROR_THRESHOLD) {
      otaPrintln("Maximum query errors hit (%d), starting search for new OTA server.\n",
                 EMBER_AF_PLUGIN_AURORA_OTA_CLIENT_QUERY_ERROR_THRESHOLD);
      restartServerDiscoveryAfterDelay();
      return;
    }
  }
...
static void continueImageDownload(void)
{
  ...
  int32u timer = EMBER_AF_PLUGIN_AURORA_OTA_CLIENT_DOWNLOAD_DELAY_MS;
  boolean send = TRUE;

  recordUpgradeStatus(BOOTLOAD_STATE_DOWNLOAD);

  if (errors >= EMBER_AF_PLUGIN_AURORA_OTA_CLIENT_DOWNLOAD_ERROR_THRESHOLD) {
...
static void continueImageVerification(EmberAfImageVerifyStatus status)
{
    ...
    emberAfDebugPrint("custom Verify 0 (%x)\r\n", status);
...
//      otaPrintln("emberAfOtaStorageCheckTempDataCallback() failed.");
    ...
    emberAfDebugPrint("custom Verify 3\r\n");

    if (status == EMBER_AF_IMAGE_VERIFY_IN_PROGRESS) {
      setTimer(EMBER_AF_PLUGIN_AURORA_OTA_CLIENT_VERIFY_DELAY_MS);
        emberAfDebugPrint("custom Verify 4\r\n");
      return;
    } else if (status == EMBER_AF_IMAGE_VERIFY_WAIT) {
        emberAfDebugPrint("custom Verify 5\r\n");
      setTimer(0);
      return;
    } else if (status == EMBER_AF_NO_IMAGE_VERIFY_SUPPORT) {
        emberAfDebugPrint("custom Verify 6\r\n");
      otaPrintln("No signature verification support, assuming image is okay.");
      customVerifyStatus = NEW_CUSTOM_VERIFY;
      setTimer(EMBER_AF_PLUGIN_AURORA_OTA_CLIENT_VERIFY_DELAY_MS);
      return;
    } else {
        emberAfDebugPrint("custom Verify 7\r\n");
...
static void askServerToRunUpgrade(boolean timeout)
{
  ...
  if (errors >= EMBER_AF_PLUGIN_AURORA_OTA_CLIENT_UPGRADE_WAIT_THRESHOLD) {
...
static EmberAfStatus imageBlockResponseParse(int8u* buffer, int8u index, int8u length)
{
  ...
  int32u currentOffset;
  int8u dataSize;
  int32u timerMs = (EMBER_AF_PLUGIN_AURORA_OTA_CLIENT_DOWNLOAD_DELAY_MS >= minBlockRequestPeriod
                    ? EMBER_AF_PLUGIN_AURORA_OTA_CLIENT_DOWNLOAD_DELAY_MS
